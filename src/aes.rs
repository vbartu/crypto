...
fn key_expansion(prev_key: &[u8; 16], round: usize) -> Box<[u8; 16]> {
    ...
    for i in 0..4 {
        ...
        new_key[i] = S_BOX[new_key[i] as usize];
        new_key[i] ^= prev_key[i];
    }

    // Round constant
    new_key[0] ^= R_CONST[round-1];
...
pub fn aes_128_encrypt(plaintext: &[u8; 16], key: &[u8; 16]) -> Box<[u8; 16]> {
    ...
    for i in 0..AES_128_ROUNDS-1 {
        sub_bytes(&mut state, &S_BOX);
        shift_rows(&mut state);
        mix_columns(&mut state, &MC_MATRIX);
    ...
    sub_bytes(&mut state, &S_BOX);
    shift_rows(&mut state);
    expanded_key = key_expansion(&expanded_key, AES_128_ROUNDS);
    add_round_key(&mut state, &expanded_key);

    state
}

pub fn aes_128_decrypt(ciphertext: &[u8; 16], key: &[u8; 16]) -> Box<[u8; 16]> {
    ...
    for i in 0..9 {
        ...
        mix_columns(&mut state, &INV_MC_MATRIX);
    }
...
const R_CONST: [u8; 10] = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36];


const MC_MATRIX: [[u8; 4]; 4] = [
    [0x2, 0x3, 0x1, 0x1],
    [0x1, 0x2, 0x3, 0x1],
    [0x1, 0x1, 0x2, 0x3],
    [0x3, 0x1, 0x1, 0x2]
